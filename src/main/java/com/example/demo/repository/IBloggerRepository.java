...
public interface IBloggerRepository extends JpaRepository<Blogger, Integer> {
	...
	@Query(value = "select b.* from blogger b join user_entity u on u.user_id=b.id where u.user_id=:id", nativeQuery = true)
	public Blogger getBloggerByUserId(@Param("id") int id);
}
